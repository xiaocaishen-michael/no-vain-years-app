...
| 反例参考 | account-settings-shell mockup PHASE 2(同链 spec B,已落地)— `apps/native/specs/account/settings-shell/design/source/`;login v2(`apps/native/specs/auth/phone-sms-auth/design/source-v2/`)PHASE 2 已 establish 的 form 视觉系统                                                                                                     |
...
**只画 modal,不重画 login form。** login.tsx 已是 PHASE 2(login v2 mockup 已 ship in 既有源 `specs/auth/phone-sms-auth/design/source-v2/`)。本 mockup 仅设计 modal overlay 部分。
...
- ❌ **不重画既有 login.tsx form**(login v2 已 ship in `specs/auth/phone-sms-auth/design/source-v2/`)— 本 mockup 仅画 freeze modal overlay 嵌入态;在 freeze-modal-active 状态下用占位 box 代表 login form 即可
