...
| T_doc1   | [Spec]            | 写 spec.md(IA + User Flow + 6 Q + 4 CL + 9 US + 20 FR + 12 SC)           | `apps/native/specs/account/settings-shell/spec.md`  | ✅ 本 PR              |
| T_doc2   | [Plan]            | 写 plan.md(10 关键决策 + 文件清单 + 测试策略 + 衔接边界)                 | `apps/native/specs/account/settings-shell/plan.md`  | ✅ 本 PR              |
| T_doc3   | [Tasks]           | 写本文件 — T1 ~ T11 实施步骤 + T_mock+ PHASE 2 占位空表                  | `apps/native/specs/account/settings-shell/tasks.md` | ✅ 本 PR              |
...
| T10 | [Smoke]                         | Playwright 真后端 prod 冒烟(release v0.2.0 验证)— 已 onboarded → ⚙️ → settings → account-security → phone → 返回 → 法规 ×2;~~退出登录 → Alert → 确定 → 跳 login~~(B 方案砍 logout,destructive case 留 dev T15 + M3 staging) | `apps/native/runtime-debug/2026-05-07-account-settings-shell-business-flow/` + 13 截图     | ✅ release v0.2.0 prod 跑通(2026-05-08 00:42 CST) |
...
| T14    | [Plan]         | plan.md UI 段从 4 边界占位回填为完整 UI 结构                                                                                                                                            | `apps/native/specs/account/settings-shell/plan.md` § UI 结构                                                       | ✅      |
...
- **2026-05-08 T10 release-verify**:server release v0.2.0 部署后跑 prod 真后端冒烟。13 截图 only-read 路径(B 方案砍 logout)— login → onboarding → tabs/profile → settings → account-security(phone mask)→ phone-detail → legal ×2 → 退栈。Network log 全 200(phone-sms-auth + GET /me + PATCH /me)。pageErrors/networkFails=0。phone 链路 server PR #139 + v0.2.0 prod 部署后真后端写入正确(account row `+8613100000007 | ACTIVE | 测试用户`)。踩坑 4 个(LibreSSL / .env.local / CORS / sms_code TTL)沉淀在 `runtime-debug/.../README.md` § 踩坑回顾。Follow-up:M3 前必须分 staging,destructive case 永远在 staging 跑,prod 留 only-read smoke。
