...
> 拿下文 prompt 全文（从 `## Prompt（拷贝）` 段以下）粘到 [claude.ai/design](https://claude.ai/design)，跑出 mockup bundle 后落 `apps/native/specs/account/settings-shell/design/source/`，下次 session 进入翻译期（PHASE 2 PR — T_mock / T12-T15）。
>
...
| 反例参考 | my-profile mockup PHASE 2(同链 spec A,已落地)— `apps/native/specs/account/profile/design/source-v1/`                                                                                  |
...
1. 解压到 `apps/native/specs/account/settings-shell/design/source/`(遵循 my-profile 同款结构)
2. 写 `apps/native/specs/account/settings-shell/design/handoff.md`(7 段,per `<meta>/docs/experience/claude-design-handoff.md` § 5):
   - Bundle 内容速览
