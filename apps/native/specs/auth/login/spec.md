...
> - 新版 mockup 由 Claude Design 单独产出（按 [`docs/experience/claude-design-handoff.md`](../../../../docs/experience/claude-design-handoff.md) § 2.1b 合一页 prompt 模板），落 `apps/native/specs/auth/login/design/`；spec.md 不直接 paste 视觉
> - **路径不变**：`apps/native/specs/auth/login/`（用户视角即"登录"，per ADR-0016 决策 1 — 无 register 心智）；`app/(auth)/login.tsx` route 不变
