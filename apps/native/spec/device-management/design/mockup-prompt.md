...
> 拿下文 prompt 全文（从 `## Prompt(拷贝)` 段以下的 ` ```text ... ``` ` 块内）粘到 [claude.ai/design](https://claude.ai/design)。运行后，把 mockup bundle 落到 `apps/native/spec/device-management/design/source/`，下次 session 进入翻译期（PHASE 3 PR — T_mock / T13 / T14 / T15 / T16）。
>
...
1. 解压到 `apps/native/spec/device-management/design/source/`（遵循 my-profile / account-settings-shell / delete-cancel 同款结构）
2. 写 `apps/native/spec/device-management/design/handoff.md`（7 段，per `<meta>/docs/experience/claude-design-handoff.md` § 5）：
   - Bundle 内容速览
