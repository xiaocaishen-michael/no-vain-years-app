...
> 拿下文 prompt 全文(从 `## Prompt(拷贝)` 段以下,即 ` ```text ... ``` ` 块内)粘到 [claude.ai/design](https://claude.ai/design),跑出 mockup bundle 后落 `apps/native/spec/delete-account-cancel-deletion-ui/design/source/`,下次 session 进入翻译期(PHASE 3 PR — T_mock / T13 / T14 / T15 / T16)。
>
...
| 反例参考 | account-settings-shell mockup PHASE 2(同链 spec B,已落地)— `apps/native/spec/account-settings-shell/design/source/`;login v2(`apps/native/spec/login/design/source-v2/`)PHASE 2 已 establish 的 form 视觉系统                                                                                                                     |
...
**只画 modal,不重画 login form。** login.tsx 已是 PHASE 2(login v2 mockup 已 ship in 既有源 `spec/login/design/source-v2/`)。本 mockup 仅设计 modal overlay 部分。
...
- ❌ **不重画既有 login.tsx form**(login v2 已 ship in `spec/login/design/source-v2/`)— 本 mockup 仅画 freeze modal overlay 嵌入态;在 freeze-modal-active 状态下用占位 box 代表 login form 即可
...
1. 解压到 `apps/native/spec/delete-account-cancel-deletion-ui/design/source/`(遵循 my-profile / account-settings-shell 同款结构)
2. 写 `apps/native/spec/delete-account-cancel-deletion-ui/design/handoff.md`(7 段,per `<meta>/docs/experience/claude-design-handoff.md` § 5):
   - Bundle 内容速览
