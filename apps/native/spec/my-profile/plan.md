...
> per ADR-0017 类 1 流程,占位 UI 阶段**不**写视觉细节(精确 px / hex / 阴影 / 自定义动画 / photo blur 沉浸式背景)。仅 4 边界:**路由 / 输入(状态切换)/ 提交事件(事件路由)/ 状态机视觉指示**。
>
> mockup 落地后(由 Claude Design 单独产出),本段重写为 PHASE 2 完整 UI 段(参考 onboarding plan.md § "UI 结构")。

### 占位 UI 4 边界条目(per FR-010)

| 边界                   | 实现                                                                                                                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **路由**               | `(tabs)/_layout.tsx` Tabs × 4 + `(tabs)/profile.tsx` profile screen + 3 placeholder pages(per 决策 6)                                                                                                        |
| **输入(状态切换)**     | profile screen 内 `useState<TabKey>('notes')` 三 tab tap 切换;底 tab bar 由 Expo Router 自动管理                                                                                                             |
| **提交事件(事件路由)** | ⚙️ → `router.push('/(app)/settings')`(per FR-005);≡ / 🔍 → noop;头像 / 背景 → noop                                                                                                                           |
| **状态机视觉指示**     | active slide tab `<Text>` 加粗 + 下划线视觉(占位裸 RN 实现:`fontWeight: 'bold'` + `<View style={{height: 2, backgroundColor: 'black'}} />`,**不引 design-tokens**);disabled 顶 nav `<Pressable opacity 0.5>` |
| **错误展示位**         | 本 spec 无网络请求 / 错误展示 — 不需要                                                                                                                                                                       |

### Banner 注释(per FR-010)
...
- 头像 / 背景 placeholder PNG / SVG 资源
- 三 slide tabs 切换过渡动画(swipe 行为也 PHASE 2 决)
- 顶 nav ≡ / 🔍 disabled 反馈形式(toast vs 仅视觉 disabled)
- 底 tab bar 切换过渡动画 / lazy load 视觉
- photo blur / 沉浸式背景视觉
- 关注 / 粉丝行视觉布局(横排 / 是否带分隔)
- pull-to-refresh
- 暗色模式 / 主题切换
