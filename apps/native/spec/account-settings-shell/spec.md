...
参考截图归档于 spec A 的 [`apps/native/spec/my-profile/design/inspiration/`](../my-profile/design/inspiration/),本 spec 直接引用,不重复归档:
...
| FR-010 | 手机号 mask 函数 `maskPhone(phone: string \| null): string`(generic 实现,per CL-002):保留 phone 字符串前缀所有非数字字符(`+` / 空格 / 破折号),提取数字部分 → 保留首 N 位(国码 + 本地号首 3,N 通常 = 国码长度 + 3)+ 末 4 位 + 中段所有数字 mask 为 `****`;输入 null → 输出 `'未绑定'`;典型: `+8613812345678` → `+86 138****5678`、`+15551234567` → `+1 555***4567`(美国国码 1 位 + 本地号首 3 + 末 4);实现位置 plan.md 决(候选 `apps/native/lib/format/phone.ts` 或 `packages/auth` 内)                                                                                                 |
